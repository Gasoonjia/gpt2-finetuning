...
class MixoutModule(torch.nn.Module):
    def __init__(self, p: float = 0.5, inplace: bool = False) -> None:
        super(MixoutModule, self).__init__()
        if p < 0 or p > 1:
            raise ValueError("mixout probability has to be between 0 and 1, "
                             "but got {}".format(p))
        self.p = p
        self.inplace = inplace
    
    def forward(self, input: Tensor) -> Tensor:
        return Mixout(input, self.p, self.training, self.inplace)
